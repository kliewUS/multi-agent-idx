...
export interface MarketRow {
    ...
    sales: any;
    avg_price: any;
    avg_price_per_sqft: any;
    avg_dom: any;
    list_to_close_pct: any;
...
export async function emailDraftAgent(userId: string) {
...
export async function emailDenyAgent(userId: string) {
    const session = getSession(userId);
...
export async function emailApproveAgent(userId: string) {
    const session = getSession(userId);
    // If there's no draft, return an error response. Don't bother updating the session.
    if(session.draft && session.status == "pending_approval" ){
...
export async function formatCombinedResponse(userId: string, listing: any, stats: any) {
    ...
    if (typeof listing == "string" 
        ...
        updateSession(userId, {lastMarketResults: JSON.parse(stats as string)});
