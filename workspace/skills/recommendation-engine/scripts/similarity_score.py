...
def calculate_similarity_score(
    target: dict,
    candidate: dict,
    target_emb: list[float], #Where do we get the embeddings?
    ...
    # Semantic similarity (40% of total score) - Change this it relies in the Embedding model instead.
    sem_sim = cosine_similarity(np.array(target_emb).reshape(1,-1),np.array(candidate_emb).reshape(1,-1))[0][0]
    score += sem_sim * 40
