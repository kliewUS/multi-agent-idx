...
//             body += chunk;
//         });
//         // Once the entire payload is loaded into memory
//         req.on("end", async () => {
//             try {
//                 const payload = JSON.parse(body || "{}");
//                 const { userId, incomingFilters, pageNum = 1, limit = 10 } = payload;
//                 if (!userId) {
//                     res.writeHead(400, { "Content-Type": "application/json" });
//                     return res.end(JSON.stringify({ error: "Missing required userId parameter." }));
//                 }
//                 // 2. Clean the incoming data extracted by your LLM layer
...
//                 );
//                 console.log(`Extracted Fields: ${JSON.stringify(extractedFields, null, 2)}`)
//                 // 3. Update and fetch from the in-memory Map (survives across requests!)
//                 updateSession(userId, extractedFields);
//                 const activeSession = getSession(userId);
//                 console.log(`Current User Id: ${userId}`);
//                 console.log(`Current Session: ${JSON.stringify(activeSession, null, 2)}`);
//                 // checkSessionInfo(userId, activeSession);
//                 // 4. Slot-Filling State Machine (Approach 1)
//                 if (!activeSession.city) {
//                     updateSession(userId, { conversationStep: 0 });
...
//                     }));
//                 }
//                 // 5. All criteria are filled -> Run query and transition step state to complete
...
//                 return res.end(JSON.stringify({ error: "Internal server error processing query." }));
//             }
//         });
//     } else {
//         // Catch-all router error handler for non-matching URLs/Methods
